...
class UsuarioRouter {
  ...
  private routers () {
    ...
    this.router.get('/', routerMiddleware.role([PerfilUtils.Gerente, PerfilUtils.Coordenador, PerfilUtils.Funcionário, PerfilUtils.Administrador]), controller.all)
    this.router.post('/', controller.create)
    this.router.get('/:id', routerMiddleware.role([PerfilUtils.Administrador, PerfilUtils.Gerente, PerfilUtils.Coordenador]), controller.find)
    this.router.post('/:id/edit', routerMiddleware.role([PerfilUtils.Administrador]), controller.update)
    this.router.post('/:id/validar', routerMiddleware.role([PerfilUtils.Administrador, PerfilUtils.Gerente, PerfilUtils.Coordenador]), controller.validar)
